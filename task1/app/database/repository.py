...
class Repository:
    ...
    def load(self, name, path):
        ...
        data, input_msg = input_instrument.read_json_s(
            path
        )  # using own input check and read function
    ...
    def checkFilesAndDb(self):
        ...
        if not (exists):
            print(
                "Database is not ready to use, missing prepared database, use load function to create a database with the data."
            )
            return False
